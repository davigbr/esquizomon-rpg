...
export function openTaskForm(task?: Task, initialType?: TaskType): void {
  ...
  openModal(`
    ...
    <form data-form-tarefa>
      <div class="form-group">
        ...
        <input class="field" name="titulo" value="${escape(task?.title ?? '')}" placeholder="Ex.: revisar fichas do mestrado" autofocus required />
      </div>

      <div class="form-group">
        <label>Tipo de tarefa</label>
        <div class="options-type" data-tipo-opcoes>
          ${TASK_TYPES.map(
            (t) => `
            <div class="option-type${t.id === type ? ' selected' : ''}" data-tipo="${t.id}" role="button" tabindex="0">
      ...
      <div class="form-group">
        ...
        <select class="field" name="dificuldade">
          ${DIFFICULTY_OPTIONS(difficulty)}
        </select>
        <small>Quanto mais difícil, mais XP a tarefa vale (e mais dano causa se falhar).</small>
      </div>

      <div class="form-group" data-campo-agenda>
        <label>Repetição</label>
        <select class="field" name="repeticao" data-repeticao-select>
        ...
        <div class="chips" data-dias-chips ${repeat === 'semana' ? '' : 'hidden'}>
          ${WEEKDAYS.map((d, i) => {
            const active = agenda.days.includes(i)
            return `<button type="button" class="chip${active ? ' active' : ''}" data-dia="${i}">${d}</button>`
          }).join('')}
        </div>
        <input class="field" name="dias-mes" data-dias-mes placeholder="Ex.: 1, 15, 30" value="${escape((agenda.daysOfMonth ?? []).join(', '))}" ${repeat === 'mes' ? '' : 'hidden'} />
        <small data-agenda-dica>${repeat === 'mes' ? 'Dias do mês separados por vírgula.' : 'Recorrente sem dia marcado vale todos os dias.'}</small>
      </div>

      <div class="form-group" data-campo-sinal hidden>
        <label>Sinal do hábito</label>
        <select class="field" name="sinal">
      ...
      <div class="form-group" data-campo-due hidden>
      ...
      <div class="form-group">
        <label>Notas</label>
        <textarea class="field" name="notas" placeholder="Detalhes, contexto, anotações…">${escape(task?.notes ?? '')}</textarea>
  ...
  const options = form.querySelectorAll('[data-tipo]')
  const agendaField = form.querySelector('[data-campo-agenda]') as HTMLElement
  const signField = form.querySelector('[data-campo-sinal]') as HTMLElement
  const dueField = form.querySelector('[data-campo-due]') as HTMLElement
  const typeInput = form.querySelector<HTMLInputElement>('input[name="tipo"]')!
  const repeatSelect = form.querySelector<HTMLSelectElement>('[data-repeticao-select]')!
  const dayChips = form.querySelector('[data-dias-chips]') as HTMLElement
  const monthDays = form.querySelector<HTMLInputElement>('[data-dias-mes]')!
  ...
  options.forEach((o) => {
    o.addEventListener('click', () => applyType(o.getAttribute('data-tipo') as TaskType))
    o.addEventListener('keydown', (e) => {
      const ev = e as KeyboardEvent
      if (ev.key === 'Enter' || ev.key === ' ') {
        ev.preventDefault()
        applyType(o.getAttribute('data-tipo') as TaskType)
      }
    })
  })
  ...
  form.addEventListener('submit', (e) => {
    ...
    const difficulty = (form.querySelector<HTMLSelectElement>('select[name="dificuldade"]')!.value ?? 'facil') as Difficulty
    const notes = (form.querySelector<HTMLTextAreaElement>('textarea[name="notas"]')!.value ?? '').trim()
    ...
    const sign =
      currentType === 'habito'
        ? ((form.querySelector<HTMLSelectElement>('select[name="sinal"]')!.value ?? 'positivo') as Task['sign'])
        : undefined
